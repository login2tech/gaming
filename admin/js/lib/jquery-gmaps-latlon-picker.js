...
$.fn.gMapsLatLonPicker = (function() {
	...
	_self.params = {
		...
		strings : {
			...
			error_empty_field : "Couldnt find coordinates for this place",
	...
	var setPosition = function(position) {
		...
		$(_self.vars.cssID + ".gllpZoom").val( _self.vars.map.getZoom() ).trigger('input');
		$(_self.vars.cssID + ".gllpLongitude").val( position.lng() ).trigger('input');
		$(_self.vars.cssID + ".gllpLatitude").val( position.lat() ).trigger('input');
	...
	var getLocationName = function(position) {
		...
		_self.vars.geocoder.geocode({'latLng': latlng}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK && results[1]) {
				$(_self.vars.cssID + ".gllpLocationName").val(results[1].formatted_address).trigger('input');
			} else {
				$(_self.vars.cssID + ".gllpLocationName").val("").trigger('input');
			}
