...
class ApplyStaff extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      is_loaded: false,
  ...
  loadData() {
    Fetcher.get(
      '/api/admin/listPaged/staff_applications?page=' + this.state.page
    )
      .then(resp => {
        if (resp.ok) {
          this.setState({
  ...
  deleteItem(id) {
    ...
    this.setState(
      ...
      () => {
        Fetcher.post('/api/admin/delete/staff_applications', {id: id})
          .then(resp => {
            this.setState({
              ['update_' + id]: false
            });
        ...
        <div className="panel">
          <div className="panel-body">
            <h2 style={{padding: 0, margin: 0}}>
              Submissions for <code>Apply for Staff</code> Form
            </h2>
          </div>
        </div>
        <div className="panel">
...
export default connect(mapStateToProps)(ApplyStaff);
